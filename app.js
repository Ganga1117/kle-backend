...
let MONGODB_URL = 'mongodb+srv://gangammak333:tnskqCNySaiXCOZS@cluster0.atqrbmh.mongodb.net/?retryWrites=true&w=majority'
...
app.post('/login',async(req,res)=>{
    try{
        ...
        if(!user){
            return res.status(400).json({
                message:"user is not registration,please register first"
            })
        }
        // compare the entered paasword
...
app.get('/product/:id',async(req,res)=>{
    try{
        ...
        let {token} = req.headers;
        let decodedToken = jwt.varify(token,"supersecret");
...
// task-9 create route to add product cart
app.post('/cart/add',async(req,res)=>{
    try{
        ...
        // find the product and add price in total
        ...
        if(user.cart){
            ...
            // extracing product IDS from the existing cart
        ...
        return res.status(200).json({
            message:"cart upadated successfully",
            cart:cart
        })
    }catch(error){
          console.log(error);
...
app.delete('/cart/product/delete',async(req,res)=>{
    try{
        ...
        // findIndex() serches for the product6 in cart
